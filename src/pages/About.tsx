...
export default function About() {
  return (
    <main className="pt-32 pb-24 container mx-auto px-4 max-w-7xl min-h-[60vh]">
      <div className="flex flex-col md:flex-row gap-12 items-center mb-20">
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl font-heading font-extrabold text-white mb-6 leading-tight">
            Our Shantiniketan School <br /> <span className="bg-brand-ocean-teal text-white px-2 rounded-lg whitespace-nowrap">Inspires You More.</span>
          </h1>
      ...
      <div className="bg-brand-ocean-dark rounded-3xl p-10 md:p-16 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="grid md:grid-cols-2 gap-12 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <div className="w-16 h-16 bg-brand-ocean-dark rounded-2xl flex items-center justify-center text-3xl font-extrabold text-white mb-6">1</div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
            <div className="w-16 h-16 bg-brand-ocean-dark rounded-2xl flex items-center justify-center text-3xl font-extrabold text-white mb-6">2</div>
      ...
      <div className="mt-24 mb-10 text-center">
        <div className="inline-block bg-brand-ocean-teal px-4 py-1.5 rounded-full text-white font-bold text-sm tracking-wider uppercase mb-4">
          Our Teachers
        </div>
        <h2 className="text-4xl md:text-5xl font-heading font-extrabold text-white mb-6">
          Meet With Our <span className="bg-brand-ocean-teal text-white px-2 rounded-lg whitespace-nowrap">Highly Skilled Management</span>
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-20 px-4">
        {[
          ...
          <motion.div key={i} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-brand-ocean-navy rounded-[2rem] p-4 shadow-xl shadow-brand-darkblue/5 border border-brand-ocean-cerulean/50 group hover:-translate-y-2 transition-transform duration-500">
            ...
            <div className="text-center pb-2">
              <h4 className="text-lg font-bold font-heading text-white mb-1">{staff.name}</h4>
              <p className="text-sm font-bold text-brand-ocean-teal uppercase tracking-wider">{staff.role}</p>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mt-20">
        {[
          ...
          <motion.div
            ...
            className="bg-brand-ocean-navy p-8 rounded-3xl shadow-xl shadow-zinc-200/50 border border-brand-ocean-cerulean/50/50 text-center hover:-translate-y-2 transition-transform duration-500 relative overflow-hidden group"
          >
            ...
            <h4 className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-brand-ocean-dark to-brand-cream mb-3 drop-shadow-sm">{stat.value}</h4>
            <p className="text-blue-100 font-bold uppercase tracking-widest text-xs relative z-10">{stat.label}</p>
          </motion.div>
        ))}
      </div>
    </main>

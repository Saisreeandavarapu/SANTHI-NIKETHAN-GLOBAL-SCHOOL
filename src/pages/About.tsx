...
export default function About() {
  return (
    <main className="pt-32 pb-24 container mx-auto px-4 max-w-7xl min-h-[60vh] bg-brand-primary">
      <div className="flex flex-col md:flex-row gap-16 items-center mb-28 px-4">
        <motion.div
        ...
        >
          ...
          <h1 className="text-4xl md:text-7xl font-heading font-black text-white mb-10 leading-[1.1] tracking-tight">
            Our Shantiniketan School <br /> <span className="text-brand-accent">Inspires You More.</span>
          </h1>
        ...
        >
          ...
          <img src="https://shantiniketanschool.com/public/theme/assets/img/about/02.png" alt="Students learning" className="rounded-[2.5rem] object-cover h-80 w-full shadow-2xl border border-white/5 relative z-10" />
          <img src="https://shantiniketanschool.com/public/theme/assets/img/about/03.png" alt="School environment" className="rounded-[2.5rem] object-cover h-80 w-full shadow-2xl border border-white/5 mt-12 relative z-10" />
        </motion.div>
      </div>

      <div className="bg-brand-secondary/30 backdrop-blur-3xl rounded-[4rem] p-12 md:p-20 text-white shadow-[0_40px_100px_rgba(0,0,0,0.3)] relative overflow-hidden border border-white/5 mx-4">
        ...
        <div className="grid md:grid-cols-2 gap-16 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="group">
            <div className="w-16 h-16 bg-brand-primary border border-brand-accent/30 rounded-2xl flex items-center justify-center text-3xl font-black text-brand-accent mb-8 shadow-xl group-hover:scale-110 transition-transform duration-500">1</div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="group">
            <div className="w-16 h-16 bg-brand-primary border border-brand-accent/30 rounded-2xl flex items-center justify-center text-3xl font-black text-brand-accent mb-8 shadow-xl group-hover:scale-110 transition-transform duration-500">2</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-28 px-4">
        {[
          ...
          <motion.div key={i} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-brand-secondary/20 backdrop-blur-2xl rounded-[3rem] p-5 border border-white/5 group hover:-translate-y-3 transition-all duration-700 shadow-[0_30px_60px_rgba(0,0,0,0.3)]">
            <div className="rounded-[2.2rem] overflow-hidden mb-8 aspect-square bg-brand-primary relative">
              <img src={staff.img} alt={staff.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 grayscale group-hover:grayscale-0" />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 px-4">
        {[
          ...
          >
            ...
            <h4 className="text-5xl font-black text-brand-accent mb-4 tracking-tighter group-hover:scale-110 transition-transform duration-500">{stat.value}</h4>
            <p className="text-zinc-500 font-black uppercase tracking-[0.2em] text-[10px] relative z-10">{stat.label}</p>
          </motion.div>
        ))}
      </div>
    </main>
